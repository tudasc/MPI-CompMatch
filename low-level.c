...
void b_send(MPIOPT_Request *request) {
	...
	if (__builtin_expect(request->flag == request->operation_number * 2 + 1,
			0)) {
...
#endif
		ucs_status_t status = ucp_put_nbi(request->ep, request->buf,
				request->size, request->remote_data_addr,
		...
		ucs_status_t status = ucp_put_nbi(request->ep, &request->flag_buffer,
				sizeof(int), request->remote_flag_addr,
				request->remote_flag_rkey);
...
void e_send(MPIOPT_Request *request) {
	...
	int count = 0;
	// busy wait
	while (__builtin_expect(request->flag < request->operation_number * 2 + 2 && !flag_comm_abort,0)) {
		++count;
		ucp_worker_progress(mca_osc_ucx_component.ucp_worker);
...
#endif
		ucs_status_t status = ucp_get_nbi(request->ep, (void*) request->buf,
				request->size, request->remote_data_addr,
				request->remote_data_rkey);
...
void e_recv(MPIOPT_Request *request) {
	...
	if (__builtin_expect(flag_comm_abort, 0)) {
		...
		if (request->flag < request->operation_number * 2 + 2) {
			//there is NO the possibility of crosstalk if the other process called free
			MPIOPT_Start(request);
			MPIOPT_Wait(request, MPI_STATUS_IGNORE);
		}
	} else
...
int MPIOPT_Wait(MPIOPT_Request *request, MPI_Status *status) {
	...
	if (request->type == SEND_REQUEST_TYPE) {
		...
		}// end while, either backup comm finished, or RDMA connection was established
...
int MPIOPT_Send_init(const void *buf, int count, MPI_Datatype datatype,
	...
	MPI_Send(&buffer_ptr, sizeof(uint64_t), MPI_BYTE, dest,
	...
	COMM_BEGIN_TAG + tag,
	MPI_COMM_WORLD, &request->rdma_exchange_request);
...
//TODO this is blocking: we may want to do it non-blocking and free all leftover ressources at the end?
