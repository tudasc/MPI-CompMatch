...
inline void DeleteMatrix(SparseMatrix &A) {
  ...
  delete [] A.mtxIndL[0];
	...
	if (A.matrixDiagonalCSC!=0)
		delete[] A.matrixDiagonalCSC;
	if (A.matrixValuesCSC[0]!=0)
		delete[] A.matrixValuesCSC[0];
	if (A.matrixValuesCSC!=0)
			delete[] A.matrixValuesCSC;
	if(A.nonzerosInCol!=0)
		delete[] A.nonzerosInCol;
	if (A.mtxCSCIndL[0]!=0)
		delete[] A.mtxCSCIndL[0];
	if (A.mtxCSCIndL!=0)
			delete[] A.mtxCSCIndL;
